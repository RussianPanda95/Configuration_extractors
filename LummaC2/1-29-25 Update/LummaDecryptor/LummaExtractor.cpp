...
void find_key_and_nonce(const std::string& filename, uint8_t* key, uint8_t* nonce) {
    ...
    uint8_t pattern[] = {
        0x93, 0x46, 0xF5, 0xF9, 0x3F, 0xE9, 0x03, 0x4F,
        0x38, 0x4D, 0x32, 0x1D, 0x30, 0xF9, 0x48, 0x77,
        0x82, 0x5A, 0x3C, 0xBF, 0x73, 0x7F, 0xDD, 0x4F,
        0x15, 0x75
    };

    std::vector<uint8_t> file_data;
    file_data.resize(file.seekg(0, std::ios::end).tellg());
    file.seekg(0);
    file.read(reinterpret_cast<char*>(file_data.data()), file_data.size());

    for (size_t i = 0; i < file_data.size() - sizeof(pattern); i++) {
        if (memcmp(file_data.data() + i, pattern, sizeof(pattern)) == 0) {
            
            memcpy(key, file_data.data() + i + sizeof(pattern), 0x20);
            
       

            memset(nonce, 0, 12);
            memcpy(nonce + 4, file_data.data() + i + sizeof(pattern) + 0x20, 8);
...
void analyze_encrypted_chunks(const uint8_t* data, size_t offset, const SectionInfo* section, const uint8_t* key, const uint8_t* nonce) {
    ...
    for (int chunk = 0; chunk < 9; chunk++) {
        size_t chunk_offset = target_offset + (chunk * 0x80);
        uint8_t chunk_data[0x80];
        memcpy(chunk_data, data + chunk_offset, 0x80);
...
void analyze_pattern_location(const std::string& filename) {
    ...
    for (size_t i = 0; i < data.size() - pattern_size - 0x28; i++) {
        if (memcmp(data.data() + i, pattern.data(), pattern_size) == 0) {


            size_t enc_offset = i + pattern_size + 0x28;
            ...
            analyze_encrypted_chunks(data.data(), enc_offset, current_section, nullptr, nullptr);
...
void decrypt(const uint8_t* data, size_t file_size, const uint8_t* key, const uint8_t* nonce, const std::vector<SectionInfo>& sections, uint32_t image_base) {
    ...
    for (size_t i = 0; i < file_size - (pattern_size + 0x28 + 0x80 * 9); i++) {
        if (memcmp(data + i, pattern, pattern_size) == 0) {
            size_t target_offset = i + pattern_size + 0x28;
            ...
            for (int chunk = 0; chunk <= 8; chunk++) {
                ...
                uint32_t counter = chunk * 2;  // 0->0, 1->2, 2->4, etc.

                uint8_t plaintext[0x80] = { 0 };
                chacha20_encrypt(plaintext, chunk_data, 0x80, key, nonce, counter);
